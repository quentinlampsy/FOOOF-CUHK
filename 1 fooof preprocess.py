...
freqs = np.array(range(2, 41-cut_off_component))
...
# Get a list of all files in the input data directory
all_files = os.listdir(input_dir)

# Filter the list to only include files with 'alpha' in the name
alpha_files = [file for file in all_files if 'alpha' in file]
...
for file in alpha_files:
    ...
    for index, row in data.iterrows():
        ...
        if index == 0:
            plot_annotated_model(fm2, annotate_aperiodic=False)
            plt.title(f'{pid} {exp} {load} annotated', fontsize=20)
            plt.savefig(os.path.join(dir_path, f'{pid}_{exp}_{load}_fml anno.png'), dpi=200)
            plt.close()
    linear_result.to_csv(os.path.join(dir_path, f'{file_name}_linear_result.csv'), index=False)
    print(f'{file_name} finished.')


# for the theta
# Specify the frequency range for theta range
...
# Filter the list to only include files with 'theta' in the name
theta_files = [file for file in all_files if 'theta' in file]
