...
dir_path = f'2 theta/'
output_dir = f'3 theta_stat/'
output_pic_dir = os.path.join(cwd, '4 theta pic')
...
mean_values.to_csv(f'{output_dir}mean_theta_params.csv', index=False)
# print(mean_values.head())

# 2 way repearted anova test on child_data grouped by values in exp and load columns
depend_var = ['CF', 'PW', 'BW', 'offset', 'exponent']
subject = 'PID'
within = ['exp', 'load']
for var in depend_var:
    print(f'ANOVA test for {var}')
    aovrm = AnovaRM(mean_values, var, subject, within)
    res = aovrm.fit()
    print(res)
...
result = pd.DataFrame(columns=['PID', 'age', 'exp', 'CF_diff', 'PW_diff', 'BW_diff', 'offset_diff', 'exponent_diff'])
...
# print(result.head())
result.to_csv(f'{output_dir}mean_theta_delta_nback_params.csv', index=False)
...
merged_df.to_csv(f'{output_dir}/p+ap_theta_load.csv', index=False)
...
mean_df.to_csv(f'{output_dir}/mean_theta_load.csv')
# print(mean_df.head())
...
img = Image.open(os.path.join(cwd, 'montage pic', 'theta.png')).convert('LA')
...
for i, (ax, (exp, age)) in enumerate(zip(axs, conditions)):
    ...
    if i not in [1, 3]:  # Dont plot y title for the right side of the subplot
        ax.set_ylabel(r'Power ($\mu V^{2})$', fontsize=20)
    # Get the current x-axis labels
    labels = [item.get_text() for item in ax.get_xticklabels()]

    # Remove the 'V' string from each label
    labels = [label.replace('V', '') for label in labels]
    ...
    fig.text(0.04, 0.5, 'Midfrontal Area', va='center', rotation='vertical', fontsize=28)
    ...
    if i >= 2:
         cell_text = [[r'$\Delta ap_{off}$', round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'offset_diff'].mean(), 2)],
            ...
            [r'$\Delta \theta_{pw}$', str(round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'PW_diff'].mean(), 2)) + '*']]
    else:
        cell_text = [[r'$\Delta ap_{off}$', round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'offset_diff'].mean(), 2)],
                [r'$\Delta ap_{exp}$', round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'exponent_diff'].mean(), 2)],
                # [r'$\Delta \theta_{cf}$', round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'CF_diff'].values[0], 2)],
                [r'$\Delta \theta_{pw}$', round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'PW_diff'].mean(), 2)]]
                #  [r'$\Delta \theta_{bw}$', round(result.loc[(result['age'] == age) & (result['exp'] == exp), 'BW_diff'].values[0], 2)]]
    ...
    for key, cell in table.get_celld().items():
        t_list = [off_t, exp_t, pw_t]
        p_list = [off_p, exp_p, pw_p]
...
plt.savefig(os.path.join(output_pic_dir, f'Fig4 theta.png'), dpi=500, bbox_inches='tight')
# plt.show()


###################################################
# plot the aperiodic power load in Fig. 2 #
###################################################
...
merged_df.to_csv(f'{output_dir}/ap_theta_load.csv', index=False)
# calcualte the mean value for each column startswith V by groups_mean
groups_mean = merged_df.groupby(['age', 'load', 'exp'])
...
plt.savefig(os.path.join(output_pic_dir, f'Fig2 a.png'), dpi=500, bbox_inches='tight')
...
merged_df.drop(columns=[f'V{i}_p' for i in range(2, max_Hz+1)] + [f'V{i}_ap' for i in range(2, max_Hz+1)], inplace=True)
# print(merged_df.head())   
merged_df.to_csv(f'{output_dir}/p_theta_load.csv', index=False)
# calcualte the mean value for each column startswith V by groups_mean
groups_mean = merged_df.groupby(['age', 'load', 'exp'])
...
plt.savefig(os.path.join(output_pic_dir, f'Fig2 g.png'), dpi=500, bbox_inches='tight')